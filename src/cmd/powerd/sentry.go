...
func (controller Sentry) doCmd(cmd string) (data string, err os.Error) {
	...
	if err != nil {
		err = os.NewError(fmt.Sprintf("Cannot connect to %s", address))
		return 
	}
...
func (controller Sentry) retryDoCmd(cmd string, retries int) (data string, err os.Error) {
	for i := 0; i < retries; i++ {
...
func (controller Sentry) Status() (status map[string]PortStatus, err os.Error) {
...
func (controller Sentry) On(hostname string) (err os.Error) {
	outlet, found := controller.Outlets[hostname]
	if !found {
		err = os.NewError(fmt.Sprintf("No outlet for host %s", hostname))
		return
	}
...
func (controller Sentry) Off(hostname string) (err os.Error) {
	outlet, found := controller.Outlets[hostname]
	if !found {
		err = os.NewError(fmt.Sprintf("No outlet for host %s", hostname))
		return
...
func (controller Sentry) Reboot(hostname string) (err os.Error) {
	outlet, found := controller.Outlets[hostname]
	if !found {
		err = os.NewError(fmt.Sprintf("No outlet for host %s", hostname))
		return
